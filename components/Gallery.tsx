    ...
    <div className="w-full pt-12 pb-4 md:py-20 bg-slate-900 border-t border-slate-800">
      ...
      <div className="relative w-full group/carousel overflow-hidden">
        ...
        <div 
          ...
          className={`
            ${isMobile 
              ? "grid grid-rows-2 grid-flow-col gap-4 overflow-x-auto scrollbar-hide px-4 pr-[40vw] touch-pan-x" 
              : "flex overflow-x-auto scrollbar-hide px-6 gap-6 md:gap-8 items-center"
            }
            pb-6 relative
          `}
        ...
        >
          {displayImages.map((img, index) => (
            <div 
              ...
              className={`
                flex-none relative shadow-2xl rounded-sm overflow-hidden border border-slate-800 cursor-pointer transition-all duration-500
                ${isMobile 
                  ? "w-[70vw] h-[30vh] even:translate-x-[50%]" 
                  : "w-[85vw] md:w-[60vw] aspect-[16/9]"
                }
                hover:scale-[1.01]
              `}
            >
              ...
              <div className="absolute bottom-0 left-0 w-full p-4 md:p-8">
                <span className="bg-gold-500/90 text-slate-900 text-[10px] md:text-xs font-bold uppercase tracking-widest py-1 px-2 md:px-3 mb-2 md:mb-3 rounded-sm inline-block">
                  {img.category}
                </span>
                <h3 className="text-xl md:text-4xl font-serif text-white font-bold drop-shadow-lg leading-tight">
                  {img.title}
                </h3>
              </div>
            </div>
      ...
      {selectedProjectIndex !== null && selectedImage && (
        <div className={`fixed inset-0 z-50 bg-slate-900 overflow-y-auto animate-[fadeIn_0.4s_ease-out]`}>
            <div className="fixed top-0 left-0 w-full z-[60] p-6 flex justify-between items-start pointer-events-none">
                <div className="bg-slate-900/40 backdrop-blur-md px-4 py-2 rounded-full border border-white/10 opacity-0 md:opacity-100 transition-opacity">
                    <span className="text-white text-xs font-bold uppercase tracking-widest">{selectedImage.category}</span>
                </div>
